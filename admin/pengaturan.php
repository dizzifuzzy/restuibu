...
$petugas = $conn->query("SELECT * FROM petugas WHERE id_petugas = '$idpetugas'");
$petugas    = $petugas->fetch_assoc(); 
$rekening = $conn->query("SELECT * FROM rekening");
$kegiatan_rutin = $conn->query("SELECT * FROM kegiatan_rutin");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title><?=$info['nama_yayasan'];?> - Pengaturan</title>
...
<?php include "petugas.phtml";?>

          </div>
          <div class="row">
<?php include "rekening.phtml";?>
          </div>
        </div>
      <!-- Footer -->
