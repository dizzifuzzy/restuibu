...
$berita = $conn->query("SELECT * FROM berita INNER JOIN detail_berita ON berita.id_berita = detail_berita.id_berita WHERE berita.id_berita = '$id'");
$berita    = $berita->fetch_assoc(); 
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title><?=$info['nama_yayasan'];?> - Ubah Data Berita</title>
            ...
            <div class="col-lg-12">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-info">Ubah Data Berita</h6>
                </div>
                <div class="card-body">
              <form class="user" action="" method="post" enctype="multipart/form-data">
                <div class="form-group">
                  <label class="col-sm-12 control-label">Judul Berita :</label>
                  <input type="text" name="judulberita" value="<?=$berita['judul_berita'];?>" class="form-control form-control-user">
                </div>
                <div class="form-group">
                  <label class="col-sm-12 control-label">Isi Berita :</label>
                  <textarea name="isiberita" class="form-control form-control-user"><?=$berita['isi_berita'];?></textarea>
                </div>
                <div class="form-group">
                  <label class="col-sm-12 control-label">Gambar :</label>
                  <center><img src="../imgberita/<?=$berita['gambar_berita'];?>" height="500" width="650"></center>
                 ...
                 $idx = strpos($_FILES['gambar']['name'],'.');
                  ...
                  if($idx){
                    ...
                    if ($_FILES["gambar"]["error"] > 0){
                      ...
                      $conn->query("UPDATE berita SET judul_berita='$judul' WHERE id_berita='$id'");
                      $conn->query("UPDATE detail_berita SET gambar_berita='$file_name', isi_berita='$isi' WHERE id_berita='$id'");
                      echo '            <script>
                window.alert("data berita berhasil diubah.");
                window.location = "ubahberita.php?id='.$id.'";
            </script>';
                    }
                  }else{
                    ...
                    $conn->query("UPDATE berita SET judul_berita='$judul' WHERE id_berita='$id'");
                    $conn->query("UPDATE detail_berita SET isi_berita='$isi' WHERE id_berita='$id'");
                      echo '            <script>
                window.alert("data berita berhasil diubah.");
                window.location = "ubahberita.php?id='.$id.'";
            </script>';
